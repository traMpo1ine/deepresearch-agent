...
README、PROJECT_DESIGN、实验报告、showcase、复试材料可能逐渐不同步。如果没有一个验收入口，后续改动很容易漏掉关键产物，比如忘记保留 `metrics.json`、忘记更新 `RESUME_NOTES.md`，或者 README 里的边界说明被删掉。
...
- 检查 README、PROJECT_DESIGN、PROJECT_COMPLETION_LOG、RESUME_NOTES 是否包含关键边界表达。
...
README、PROJECT_DESIGN、EXPERIMENTS、RESUME_NOTES 分别解决不同问题，但它们比较分散。项目完成后，需要一份可以从头读到尾的正式结项报告。

### Problem

如果未来学习时只看 README，会知道怎么运行，但不一定能理解每个模块为什么这样设计；如果只看 BUILD_LOG，会看到过程，但不够像最终交付文档；如果只看 EXPERIMENTS，又缺少架构和复试表达。
...
项目已经有 `RESUME_NOTES.md` 和最终 bullet，但复试时不能只背结论。每条简历说法都应该能反查到真实代码、测试、命令、实验产物和边界，否则容易变成“看起来很完整，但问细节答不上来”。

### Problem

原来的 `RESUME_NOTES.md` 只列了模块级代码路径，比如 DAG/并发、记忆/检索、验证/修复。这个粒度不够细：
