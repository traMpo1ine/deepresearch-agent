...
- Local corpus profiles：`offline_agent_docs`、`resume_agent_docs`、`paper_reading_docs` 可从本地 Markdown/TXT/HTML 构建 Searcher-compatible JSONL，Web Demo 可选择 corpus profile，增强本地知识库 RAG 真实感。
...
- 新增 local corpus profile 模式，将本地 Markdown/TXT/HTML 资料构建为 Searcher-compatible JSONL，并在 Web Demo 中选择不同知识库 profile，展示本地企业知识库式 RAG。
...
- 构建 local corpus profile 知识库模式，将本地资料目录转成可检索 JSONL corpus，并在 Demo 中支持 `offline_agent_docs`、`resume_agent_docs`、`paper_reading_docs` 三类场景切换。
